...
describe('esitoGirone (sempre 3 set)', () => {
  ...
  it('non è completa con meno di 3 set validi', () => {
    expect(esitoGirone([s(21, 15), s(21, 10)], bo3).completa).toBe(false)
  })
  it('con 3 set: 2-1 conta i set e assegna il vincitore', () => {
    const o = esitoGirone([s(21, 15), s(10, 21), s(15, 12)], bo3)
    expect(o).toEqual({ vincitore: 'A', setA: 2, setB: 1, completa: true })
  })
  it('3-0 valido', () => {
    const o = esitoGirone([s(21, 5), s(21, 9), s(15, 3)], bo3)
    expect(o).toEqual({ vincitore: 'A', setA: 3, setB: 0, completa: true })
  })
  it('il terzo set usa il tie-break (15): 14-12 non è ancora set valido', () => {
    const o = esitoGirone([s(21, 15), s(10, 21), s(14, 12)], bo3)
    expect(o.completa).toBe(false)
  })
})
